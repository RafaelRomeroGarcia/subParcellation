function backtrackingParcellation(subject,surface,hemi,vecinos)
path_area= [subject 'surf/'  hemi '.white.avg.area.mgh'];
path_annot=[subject 'label/' hemi '.aparc.annot'];
path_new_annot=[subject 'label/' hemi int2str(surface) '.new.aparc.annot'];
...
for i=1:numLabels,
    ...
    if entra && (not (strcmp(colortable.struct_names(i),'unknown') || strcmp(colortable.struct_names(i),'Corpus_callosum') || strcmp(colortable.struct_names(i),'corpuscallosum') || strcmp(colortable.struct_names(i),'Unknown'))),  
      ...
      % end
            ...
            while j<=numVertexRegAux,
                ...
                if(suma>areaFinalRegion || j==numVertexRegAux),
                    ...
                    if (tamVertRegion>0),
                       [label conexa]=esConexa(vertexRegionConexa,closeVertex,label);
                       %conexa=1;
                       if (conexa==0) 
                             label=labelSeguro;
        ...
        %colorUsado=colorUsado+1; %%%CAMBIADO
...
Write_Brain_Annotation([path_sub 'label/' lado '.new.aparc.annot'],vertices, newLabel, newColorTable);
